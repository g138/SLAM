...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        r = motor_ticks[0] * ticks_to_mm
        l = motor_ticks[1] * ticks_to_mm
        alpha = (r - l) / robot_width
        R = l / alpha
        theta = old_pose[2] 
        cx = (old_pose[0] - scanner_displacement) - ((R + robot_width/2) * sin(theta))
        cy = (old_pose[1] - scanner_displacement) - ((R + robot_width/2) * (-cos(theta)))
        theta_update = (theta + alpha) % (2 * pi)
        x = cx + ((R + robot_width/2) * sin(theta_update)) + scanner_displacement
        y = cy + ((R + robot_width/2) * (-cos(theta_update))) + scanner_displacement
        theta = theta_update
