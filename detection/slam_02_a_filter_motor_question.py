...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width):
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        R = l / alpha
        theta = old_pose[2] 
        cx = old_pose[0] - ((R + robot_width/2) * sin(theta))
        cy = old_pose[1] - ((R + robot_width/2) * (-cos(theta)))
        theta_update = (theta + alpha) % (2 * pi)
        x = cx + ((R + robot_width/2) * sin(theta_update))
        y = cy + ((R + robot_width/2) * (-cos(theta_update)))
        theta = theta_update
